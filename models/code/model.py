...
class FCModel(nn.Module):
    ...
    def get_relation(self, index, time):
        ...
        if len(relation):
            ...
            # print('debug relation.shape', relation.shape)
    ...
    def kg_attention(self, att_h, relation):
        ...
        # print('debug att_h.shape', att_h.shape)
        # print('debug relation.shape', relation.shape)
    ...
    def forward(self, seq, mask, label=None):
        ...
        # print('debug pool_output.size', pool_output.size())

        
        pool_output = pool_output.view(pool_output.size()[:-1])
        # print('debug pool_output.size', pool_output.size())
        if self.opt.use_gp:
            output = pool_output
        ...
        if label is not None:
            ...
            # print('debug pool_output.shape', pool_output.shape)
            ...
            x = seq.data.cpu().numpy()
            return out1, contributions, outputs, logit_weight, graph_att_res, x
        else:
            return out1
